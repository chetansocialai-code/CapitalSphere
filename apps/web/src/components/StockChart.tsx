...
export function StockChart({ symbol = 'RELIANCE' }: { symbol?: string }) {
  ...
  return (
    <div className="cs-card border rounded-xl p-4 shadow-lg space-y-4 font-mono text-xs">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b cs-border pb-3">
        <div className="flex items-center gap-2">
          <span className="bg-[#4DA3FF]/10 text-[#4DA3FF] border border-[#4DA3FF]/30 px-2 py-0.5 rounded font-bold text-2xs uppercase">
            NSE: {symbol}
          </span>
          <h3 className="text-sm font-bold">Interactive Candlestick & Technical Terminal</h3>
        </div>

        {/* Timeframe & Chart Controls */}
        <div className="flex items-center gap-2">
          <div className="flex cs-topbar p-0.5 rounded border cs-border text-2xs">
            {(['1D', '1W', '1M', '1Y'] as const).map((tf) => (
              <button
                key={tf}
                onClick={() => setTimeframe(tf)}
                className={`px-2 py-1 rounded transition ${timeframe === tf ? 'bg-[#4DA3FF] text-slate-950 font-bold' : 'cs-text-sub hover:text-blue-500'}`}
              >
                {tf}
              </button>
            ))}
          ...
          <div className="flex cs-topbar p-0.5 rounded border cs-border text-2xs">
            <button
              ...
              className={`p-1.5 rounded transition ${chartType === 'CANDLE' ? 'bg-[#4DA3FF] text-slate-950' : 'cs-text-sub'}`}
            ...
            <button
              ...
              className={`p-1.5 rounded transition ${chartType === 'LINE' ? 'bg-[#4DA3FF] text-slate-950' : 'cs-text-sub'}`}
      ...
      <div className="relative h-64 w-full cs-topbar border cs-border rounded-lg p-4 flex flex-col justify-between overflow-hidden">
        {/* Technical Overlays Header */}
        <div className="flex gap-4 text-3xs cs-text-sub">
          ...
          <span>VWAP: 3,012.80</span>
        </div>

        {/* SVG Chart Graphic */}
        <svg className="w-full h-44" viewBox="0 0 500 150" preserveAspectRatio="none">
          {/* Gridlines */}
          <line x1="0" y1="30" x2="500" y2="30" stroke="currentColor" strokeOpacity="0.1" strokeDasharray="3 3" />
          <line x1="0" y1="75" x2="500" y2="75" stroke="currentColor" strokeOpacity="0.1" strokeDasharray="3 3" />
        ...
        <div className="flex justify-between text-3xs cs-text-sub pt-2 border-t cs-border">
          <span>09:15 AM</span>
          <span>11:30 AM</span>
          <span>01:45 PM</span>
          <span>03:30 PM</span>
